		...
		this.oTask.addListener("checkMasterPool", function (matrixItem, taskQueue) {
    		...
    		if (networklinkCache.hasOwnProperty(tileUrl)) {
				...
				if (pixelCoveringSize >= minLodPixels && pixelCoveringSize <= maxLodPixels) {       				
                    ...
                    //for each tile, this fuction is called once
        			newKmlDatasource.load(tileUrl).then(function(dataSource) {
                        console.log("newKmlDatasource.load(tileUrl).then(function(dataSource)      is called");
//                        dataSource_name="hh-complete_Tile_67_61_footprint"
//                        
                            var entities = dataSource.entities.values; 
                        
                            for(var i =0; i < entities.length; i++)
                            {
                                //entity_name="DEHH_9d4c8b35-56b8-4fae-9228-46a2686391d8"
                                var e = entities[i];
